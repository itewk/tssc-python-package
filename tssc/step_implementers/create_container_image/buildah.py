...
|---------------------|-----------------|------------
| `generate-metadata` | `image-version`     | Version to use when building the image
...
|------------------|------------
| `image-version`      | The image ID to tag the built image with when pushing it to a local file
    ...
    def _run_step(self):
        ...
        if(self.get_step_results(DefaultSteps.GENERATE_METADATA) and \
          self.get_step_results(DefaultSteps.GENERATE_METADATA).get('container-image-version')):
            image_tag_version = self.get_step_results(DefaultSteps.GENERATE_METADATA)['container-image-version']
        else:
